...
# Colores de logging en terminal
LOG_COLORS = {
    "DEBUG": "\033[94m",     # Azul
...
def setup_logger(name: str = "corebrain", 
    ...
    if file_path:
        ...
        # For files, use formatter without colors
...
def get_logger(name: str, level: Optional[int] = None) -> logging.Logger:
    ...
    if not logger.handlers:
        ...
        if '.' in name:
            ...
            # It is a main logger, configure completely
            logger = setup_logger(name, level or DEFAULT_LEVEL)
    elif level is not None:
        # Update level only if specified
        logger.setLevel(level)
...
def set_log_level(level: Union[int, str], 
    ...
    if logger_name:
        ...
        # Also affect specific SDK loggers
